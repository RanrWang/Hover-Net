...
best_epoch = 0
model_dict = hovernet.state_dict()
pretrained_dict = torch.load('resnet50.pth')
from collections import OrderedDict
new_state_dict = OrderedDict()
for k, v in pretrained_dict.items():
    name = 'module.encoder.' + k
    new_state_dict[name] = v
pretrained_dict = {k: v for k, v in new_state_dict.items() if k in model_dict}
model_dict.update(pretrained_dict)
hovernet.load_state_dict(model_dict)
# main training loop
for i in tqdm(range(n_epochs)):
    minibatch_train_losses = []
    minibatch_train_dice = []
    # if i <=50:
    #     for k, v in hovernet.named_parameters():
    #         if "module.encoder" in k:
    #             v.requires_grad = False  # 固定参数
    #         print('epoch:',i,k, v.requires_grad)
    #         # print(k)
    # else:
    #     for k, v in hovernet.named_parameters():
    #         if "module.encoder" in k:
    #             v.requires_grad = True  # jiedong参数
    #         print('epoch:',i,k, v.requires_grad)
